...
import type { NodeBuilder, NodeConstructor, NodeTypeGuard } from "./Node.ts";
...
> {
  ...
  as<N extends Class<D, S>>(NodeType: NodeConstructor<D, S, N>): N {
  ...
  ): Generator<[Class, BaseEdge.Class[], number]> {
    function* inner(
      root: Class,
    ): Generator<[Class, BaseEdge.Class[], number]> {
      const toVisit: [Class, BaseEdge.Class[]][] = [[root, []]];
...
export const TypeGuard: NodeTypeGuard<Data, ScratchData> = {
  ...
  isScratchDataCompatible(_sData: ScratchData): _sData is ScratchData {
    return true;
  },
};
