 ...
 * The abstract class
 * {@link pt.up.fe.specs.smali.weaver.abstracts.ASmaliWeaverJoinPoint} can be
 * used to add user-defined methods and fields which the user intends to add for
 * all join points and are not intended to be used in LARA aspects.
 * 
...
public class SmaliWeaver extends ASmaliWeaver {
	...
	public boolean begin(List<File> sources, File outputDir, DataStore args) {
		...
		parser = new SmaliParser(sources);
		root = parser.parse().orElseThrow();
