...
public class ProgramJp extends AProgram {
    ...
    public ProgramJp(App program, SmaliWeaver weaver) {
        super(weaver);
    ...
    public AManifest getManifestImpl() {
        return SmaliJoinpoints.create(this.program.getManifest(), getWeaverEngine(), AManifest.class);
    }

    @Override
    public AClassNode[] getClassesArrayImpl() {
        return this.program.getClasses().stream()
                .map(node -> SmaliJoinpoints.create(node,
                        getWeaverEngine(), AClassNode.class))
